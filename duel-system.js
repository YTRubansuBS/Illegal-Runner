(() => {
  ...
  function updateGhost(s){const now=performance.now();const target=Number(meA(s)?s.distance_b:s.distance_a);const targetY=Number(meA(s)?s.y_b:s.y_a)||0;if(Number.isFinite(target)){const current=S.ghostTarget||0;const currentY=S.ghostTargetY||0;S.ghostFrom=Number.isFinite(S.ghost)?S.ghost:current;S.ghostFromY=Number.isFinite(S.opponentY)?S.opponentY:currentY;S.ghostTarget=target;S.ghostTargetY=targetY;S.ghostStart=now;S.ghostDuration=170;S.ghost=target}S.opponentAction=meA(s)?s.action_b:s.action_a;S.opponentY=targetY;S.opponentCharacter=otherCharacter(s)}
