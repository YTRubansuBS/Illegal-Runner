(() => {
  ...
  async function buyPack(type){
    ...
    if(guestMode()){
      ...
      if(type==='reward'){const r=100+Math.floor(Math.random()*901);p.coins+=r;saveLocal(p);toast('💎 +'+r+' pièces !');return}
      ...
      const id=locked[Math.floor(Math.random()*locked.length)];p[key]=[...new Set([...arr,id])];saveLocal(p);toast('🎁 Débloqué : '+(type==='world'?WORLDS[id].name:type==='character'?CHARS[id].name:OBS[id].name));renderCustomizer();return
    }
    ...
    if(type==='reward'){const r=100+Math.floor(Math.random()*901);const {error}=await sb.from('profiles').update({coins:(prof.coins|0)-cost+r}).eq('id',user.id);if(error)return toast('❌ Achat impossible.');toast('💎 +'+r+' pièces !');return}
    ...
    await getCloud();toast('🎁 Débloqué : '+(type==='world'?WORLDS[id].name:type==='character'?CHARS[id].name:OBS[id].name));renderCustomizer()
  }

  function card(html){return `<div class="card">${html}</div>`}
  async function renderCustomizer(){
    const root=$('customizerRoot');if(!root)return;await getCloud();const p=localProfile();const selected={world:p.selected_background||'city',character:p.selected_character||'runner',obstacle:p.selected_obstacle_set||'classic'}
