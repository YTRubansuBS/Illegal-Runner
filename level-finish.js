...
(() => {
  ...
  function rewardForLevel(lv){
    return Math.max(0,100*Math.ceil(lv/10))
  }

  function rewardKey(identity){
    return `illegalRunner.finishRewards.v4:${identity}`
  }
  ...
  function readPaid(identity){
    const keys=[
      rewardKey(identity.id),
      `illegalRunner.finishRewards.v3:${identity.id}`
    ]
    const all=new Set()
    for(const key of keys){
      try{
        const list=JSON.parse(localStorage.getItem(key)||'[]')
        if(Array.isArray(list)) list.forEach(v=>all.add(Number(v)))
      }catch(e){}
    }
    return all
  }

  function writePaid(identity,set){
    localStorage.setItem(rewardKey(identity.id),JSON.stringify([...set].sort((a,b)=>a-b)))
  }

  async function alreadyPaid(lv,identity){
    return readPaid(identity).has(Number(lv))
  }
  ...
  async function progressionSaysCompleted(lv,identity){
    try{
      if(identity.type==='local'){
        ...
        const highest=Number(p.highest_level||1)
        return highest>Number(lv)
      }
      ...
      return Number(data.highest_level||1)>Number(lv)
    }catch(e){
      return false
    }
  }

  async function markPaid(identity,lv){
    const set=readPaid(identity)
    set.add(Number(lv))
    writePaid(identity,set)
  }

  async function unmarkPaid(identity,lv){
    const set=readPaid(identity)
    set.delete(Number(lv))
    writePaid(identity,set)
  }

  async function grantRewardOnce(lv,reward,identity){
    if(rewardBusy)return false
    if(await alreadyPaid(lv,identity))return false

    rewardBusy=true

    // Mark BEFORE crediting so two finish callbacks cannot both pay.
    await markPaid(identity,lv)

    try{
      if(identity.type==='local'){
        let p={}
        try{p=JSON.parse(localStorage.getItem('irGuest')||'{}')}catch(e){}
      ...
      if(sup&&cfg.SUPABASE_URL&&cfg.SUPABASE_ANON_KEY&&identity.user?.id){
        ...
        if(!error&&prof){
          ...
          if(!upd.error){
            window.dispatchEvent(new CustomEvent('ir:profileChanged',{detail:{coins:next}}))
            return true
          }
        }
      }

      await unmarkPaid(identity,lv)
      return false
    }catch(e){
      await unmarkPaid(identity,lv)
      return false
  ...
  function updateFlag(){
    ...
    if(!lv||isInfinite()||ended||p<0.45){
      flag.style.display='none'
      return
    }

    const t=Math.max(0,Math.min(1,(p-0.45)/0.55))
    const farX=innerWidth*.92
    const playerX=Math.max(80,innerWidth*.20)

    flag.style.left=`${farX+(playerX-farX)*Math.pow(t,1.65)}px`
    flag.style.bottom=`${Math.max(88,Math.min(132,innerHeight*.16))}px`
    flag.style.display='block'
    flag.classList.toggle('near',t>.8)
  }

  function hideNextInInfinite(){
    document.querySelectorAll('#btnNextLevel,#nextLevel,#btnNext').forEach(b=>{
      b.style.display='none'
      b.disabled=true
    })
    document.querySelectorAll('button').forEach(b=>{
      if(/niveau\s*suivant|next\s*level/i.test(b.textContent||'')){
        b.style.display='none'
        b.disabled=true
      }
    })
  ...
  async function showFinishAtFlag(){
    ...
    // A level is already finished if it was previously paid OR progression
    // has already moved beyond this level.
    ...
    // IMPORTANT: no +100/+200 GUI when the level was already completed.
    if(alreadyCompleted){
    ...
    if(lv<300){
      ...
      next.onclick=e=>{
        ...
        finishShown=false
        const overNow=$('over')
        if(overNow)overNow.style.display='none'
        const flagNow=$('levelFinishCourseFlag')
        if(flagNow)flagNow.style.display='none'
        const b=document.querySelector(`#levelButtons button[data-level="${lv+1}"]`)
        if(b&&!b.disabled)setTimeout(()=>b.click(),50)
      }
    }
  }
