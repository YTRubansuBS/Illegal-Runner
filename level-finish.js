(() => {
  ...
  function styles(){
    ...
    s.textContent=`#levelFinishCourseFlag{position:absolute;left:92vw;bottom:96px;width:54px;height:122px;z-index:6;pointer-events:none;display:none;transform:translateX(-50%);filter:drop-shadow(0 0 10px rgba(0,229,255,.65));transition:left .08s linear}#levelFinishCourseFlag .pole{position:absolute;left:25px;bottom:0;width:5px;height:110px;border-radius:4px;background:linear-gradient(#fff,#00e5ff 45%,#1677a0);box-shadow:0 0 9px rgba(0,229,255,.8)}#levelFinishCourseFlag .flag{position:absolute;left:29px;top:4px;width:48px;height:31px;border-radius:2px 8px 8px 2px;background:linear-gradient(135deg,#00e5ff,#7df8ff 48%,#008bb8);clip-path:polygon(0 0,100% 0,78% 50%,100% 100%,0 100%);box-shadow:0 0 12px rgba(0,229,255,.8)}#levelFinishCourseFlag .flag:after{content:'🏁';position:absolute;inset:0;display:grid;place-items:center;font-size:17px}#levelFinishCourseFlag .base{position:absolute;left:13px;bottom:0;width:29px;height:8px;border-radius:50%;background:#00e5ff;box-shadow:0 0 12px rgba(0,229,255,.9)}#levelFinishCourseFlag.near .flag{animation:irFinishWave .45s ease-in-out infinite alternate}@keyframes irFinishWave{from{transform:skewY(-2deg)}to{transform:skewY(4deg)}}#levelFinishReward{margin:10px 0 0;padding:9px 12px;border-radius:12px;background:rgba(0,229,255,.10);border:1px solid rgba(0,229,255,.25)}#btnNextLevel{margin-top:10px}`
  ...
  async function grantReward(lv,reward){
    ...
    try{
      ...
      if(sup&&cfg.SUPABASE_URL&&cfg.SUPABASE_ANON_KEY){
        ...
        if(user){
          const {data:prof,error}=await client.from('profiles').select('coins').eq('id',user.id).single()
          if(!error&&prof){
  ...
  async function showFinishAtFlag(){
    ...
    if(lv<300){
      ...
      next.onclick=e=>{e.preventDefault();e.stopPropagation();finishShown=false;const overNow=$('over');if(overNow)overNow.style.display='none';const flagNow=$('levelFinishCourseFlag');if(flagNow)flagNow.style.display='none';const b=document.querySelector(`#levelButtons button[data-level="${lv+1}"]`);if(b&&!b.disabled)setTimeout(()=>b.click(),50)}
    }
  }
  function reset(){const over=$('over');if(over&&getComputedStyle(over).display==='none'){finishShown=false;rewardBusy=false}}
