...
(() => {
  ...
  function updateFlag(){styles();const game=$('game');if(!game)return;let flag=$('levelFinishCourseFlag');if(!flag){flag=document.createElement('div');flag.id='levelFinishCourseFlag';flag.innerHTML='<div class="flag"></div><div class="pole"></div><div class="base"></div>';game.appendChild(flag)}const lv=getLevel(),p=getProgress(),over=$('over'),ended=over&&getComputedStyle(over).display!=='none';if(!lv||isInfinite()||ended||p<0.45){flag.style.display='none';return}const t=Math.max(0,Math.min(1,(p-0.45)/0.55)),farX=innerWidth*.92,playerX=Math.max(80,innerWidth*.20);flag.style.left=`${farX+(playerX-farX)*Math.pow(t,1.65)}px`;flag.style.bottom=`${Math.max(88,Math.min(132,innerHeight*.16))}px`;flag.style.display='block';flag.classList.toggle('near',t>.8)}
  ...
  async function showFinishAtFlag(){const over=$('over'),card=document.querySelector('#over .over-card');if(!over||!card)return;if(isInfinite()){hideCourseFlag();hideNextInInfinite();return}if(finishShown||getLevel()<1||getProgress()<0.995)return;const lv=getLevel(),token=transitionToken,reward=rewardForLevel(lv),identity=await getIdentity();
    hideCourseFlag();if(token!==transitionToken||getLevel()!==lv||getComputedStyle(over).display==='none')return;const paid=await alreadyPaid(lv,identity);if(token!==transitionToken||getLevel()!==lv||getComputedStyle(over).display==='none')return;const alreadyCompleted=paid;finishShown=true;const dist=Math.floor(Number(String($('dist')?.textContent||'0').replace(/[^0-9.]/g,''))||0),collected=Math.max(0,Math.floor(Number(String($('runCoins')?.textContent||'0').replace(/[^0-9.]/g,''))||0));$('overTitle').textContent=lv>=300?'👑 CHAMPION !':`NIVEAU ${lv} TERMINÉ`;$('finalDist').textContent=dist;$('finalCoins').textContent=collected+(alreadyCompleted?0:reward);$('finalTime').textContent='—';over.style.display='grid';let box=$('levelFinishReward');if(!box){box=document.createElement('div');box.id='levelFinishReward';card.appendChild(box)}if(alreadyCompleted){box.style.display='none'}else{box.style.display='';box.innerHTML=`🎉 Récompense de réussite : <b>+${reward} 🪙</b><br>🪙 Pièces ramassées : <b>${collected}</b>`;}if(lv<300){let next=$('btnNextLevel');if(!next){next=document.createElement('button');next.id='btnNextLevel';next.className='primary';next.type='button';next.textContent='➡️ NIVEAU SUIVANT';const row=card.querySelector('.row');if(row)row.insertBefore(next,row.firstChild);else card.appendChild(next)}next.style.display='';next.disabled=false;next.onclick=e=>{e.preventDefault();e.stopPropagation();const nextLevel=lv+1;const newToken=++transitionToken;finishShown=false;rewardBusy=false;const overNow=$('over');if(overNow)overNow.style.display='none';const flagNow=$('levelFinishCourseFlag');if(flagNow)flagNow.style.display='none';const progressNow=$('progressBar');if(progressNow){progressNow.style.width='0%';progressNow.setAttribute('aria-valuenow','0')}if($('finalDist'))$('finalDist').textContent='0';if($('finalCoins'))$('finalCoins').textContent='0';if($('finalTime'))$('finalTime').textContent='0';if($('overTitle'))$('overTitle').textContent='NIVEAU '+nextLevel;const b=document.querySelector(`#levelButtons button[data-level="${nextLevel}"]`);if(b&&!b.disabled)setTimeout(()=>{if(newToken===transitionToken)b.click()},60)}}}
  function reset(){const over=$('over');if(over&&getComputedStyle(over).display!=='none')hideCourseFlag();if(over&&getComputedStyle(over).display==='none'){finishShown=false;rewardBusy=false;hideCourseFlag()}}
  function start(){styles();const over=$('over');if(over){new MutationObserver(()=>{if(getComputedStyle(over).display!=='none')hideCourseFlag()}).observe(over,{attributes:true,attributeFilter:['style','class']})}setInterval(()=>{reset();updateFlag();showFinishAtFlag()},50)}
