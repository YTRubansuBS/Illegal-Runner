  ...
  fetch(ORIGINAL, { cache: 'no-store' })
    ...
    .then(code => {
      ...
      const progressiveSpawn = [
        ...
        '    const r = Math.random()',
        '    const stage1 = d < 500',
        '    const stage2 = d >= 500 && d < 1000',
        '    const stage3 = d >= 1000 && d < 1500',
        '    const stage4 = d >= 1500 && d < 5000',
        '    const stage5 = d >= 5000 && d < 10000',
        '    const stage6 = d >= 10000',
        '    const diff = stage1 ? 0 : stage2 ? 0.2 : stage3 ? 0.35 : stage4 ? 0.55 : stage5 ? 0.78 : 1',
        '    const pitChance = stage1 ? 0.08 : stage2 ? 0.12 : stage3 ? 0.16 : stage4 ? 0.21 : stage5 ? 0.27 : 0.32',
        '    const comboChance = stage1 ? 0 : stage2 ? 0.10 : stage3 ? 0.18 : stage4 ? 0.28 : stage5 ? 0.40 : 0.50',
        '    if (r < (0.32 - diff * 0.10)) G.obs.push({ type: "spike", x: G.W + 40, y: gy - 40, w: 42, h: 40, destructible: true })',
        '    else if (r < (0.54 - diff * 0.08)) { const h = rand(46, 60 + diff * 65); G.obs.push({ type: "wall", x: G.W + 40, y: gy - h, w: 30, h }) }',
        '    else if (r < (0.68 - diff * 0.04)) G.obs.push({ type: "car", x: G.W + 40, y: gy - 56, w: 96, h: 56, vx: rand(20, 70 + diff * 45) })',
        '    else if (r < (0.78 - diff * 0.02)) { G.obs.push({ type: "drone", x: G.W + 40, y: gy - rand(150, 200), w: 48, h: 34, destructible: true, bob: rand(0, 6.28), baseY: 0 }); G.obs[G.obs.length - 1].baseY = G.obs[G.obs.length - 1].y }',
        '    else if (r < (0.88 - diff * 0.01)) G.obs.push({ type: "projectile", x: G.W + 40, y: gy - rand(50, 95), w: 28, h: 18, destructible: true, vx: rand(180, 260 + diff * 80) })',
        '    else if (r < 0.88 + pitChance) { const w = rand(90 + diff * 8, 125 + diff * 45); G.obs.push({ type: "pit", x: G.W + 40, y: gy, w, h: G.groundH }); if (Math.random() < comboChance) G.obs.push({ type: "spike", x: G.W + 40 + w + (stage6 ? 140 : stage5 ? 165 : 190), y: gy - 40, w: 42, h: 40, destructible: true }) }',
        '    else { const py = gy - (90 + diff * 55); G.obs.push({ type: "platform", x: G.W + 40, y: py, w: Math.max(95, 130 - diff * 20), h: 16 }); if (Math.random() < comboChance) G.obs.push({ type: "spike", x: G.W + 40 + (stage6 ? 175 : stage5 ? 195 : 220), y: gy - 40, w: 42, h: 40, destructible: true }) }',
        '    void set',
        '  }'
