...
(() => {
  ...
  fetch(ORIGINAL, { cache: 'no-store' })
    ...
    .then(code => {
      if (window.IR_LOADING) window.IR_LOADING.setStep('engine','loading','Moteur du jeu téléchargé, initialisation…')
      ...
      code = code.replace('    if (!isGuest) { await claimAutoLogin(); }', '    if (!isGuest) { await claimAutoLogin(); }\\n    if (window.IR_LOADING) window.IR_LOADING.done("daily", profile.last_login_reward === new Date().toISOString().slice(0,10) ? "Cadeau quotidien vérifié." : "Cadeau quotidien non récupéré.")\\n    refreshTop()\\n    if (window.IR_LOADING) {\\n      window.IR_LOADING.done("custom","Personnalisations chargées.")\\n      window.IR_LOADING.done("quests","État des quêtes chargé.")\\n      window.IR_LOADING.done("coins","Solde chargé : " + Number(profile.coins || 0).toLocaleString("fr-FR") + " pièces.")\\n      window.IR_LOADING.done("best","Meilleur score chargé : " + Number(profile.best_distance || 0) + " m.")\\n      window.IR_LOADING.done("level","Niveau chargé : " + Number(profile.highest_level || 1) + ".")\\n      window.IR_LOADING.finishSession()\\n    }')
...
`)
    ...
    if (window.IR_LOADING) window.IR_LOADING.done('profile','Profil et sauvegarde chargés.')
    try {
...
`;
  ...
  /* ============================================================
     GAME ENGINE`);
      ...
      if (window.IR_LOADING) window.IR_LOADING.engineDone()
    })
    .catch(err => {
      if (window.IR_LOADING) window.IR_LOADING.engineError(err && err.message ? err.message : 'Erreur de chargement du jeu.'); console.error(err); const e = document.getElementById('err'); if (e) e.textContent = 'Erreur de chargement du jeu. Recharge la page.' })
})()
