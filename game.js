...
(() => {
  ...
  fetch(ORIGINAL, { cache: 'no-store' })
    ...
    .then(code => {
      ...
      code = code.replace('    if (!isGuest) { await claimAutoLogin(); }', '    if (!isGuest) { claimAutoLogin().catch(e => console.warn("[IR] daily reward load:", e)); }; if (window.IR_LOADING) window.IR_LOADING.done("daily", profile.last_login_reward === new Date().toISOString().slice(0,10) ? "Cadeau quotidien vérifié." : "Cadeau quotidien non récupéré."); refreshTop(); if (window.IR_LOADING) { window.IR_LOADING.done("custom","Personnalisations chargées."); window.IR_LOADING.done("quests","État des quêtes chargé."); window.IR_LOADING.done("coins","Solde chargé : " + Number(profile.coins || 0).toLocaleString("fr-FR") + " pièces."); window.IR_LOADING.done("best","Meilleur score chargé : " + Number(profile.best_distance || 0) + " m."); window.IR_LOADING.done("level","Niveau chargé : " + Number(profile.highest_level || 1) + "."); window.IR_LOADING.finishSession(); }')
