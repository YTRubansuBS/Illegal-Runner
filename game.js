...
`)
      ...
      code = code.replace('if (p.y > G.H + 40) { G.lives = 0; drawHearts(); end() }', 'let inPit = false; for (const o of G.obs) { if (o.type === "pit" && p.x + 8 < o.x + o.w && p.x + p.w - 8 > o.x && p.y + p.h >= G.groundY - 2) { inPit = true; break } } if (inPit) { G.lives = 0; drawHearts(); end(); return } if (p.y > G.H + 40) { G.lives = 0; drawHearts(); end() }')
